                    ...
                    @if (!isCloud())
                        <div class="w-48">
                            <x-forms.checkbox canGate="update" :canResource="$github_app" label="Загальносистемний?"
                                helper="Якщо позначено, цей Застосунок GitHub буде доступний для всіх у цьому екземплярі AutoDeploy."
                                instantSave id="isSystemWide" />
                        </div>
                        @if ($isSystemWide)
                            <x-callout type="warning" title="Не рекомендовано">
                                Загальносистемні Застосунки GitHub є спільними для всіх команд у цьому екземплярі AutoDeploy. Це означає, що будь-яка команда може використовувати цей Застосунок GitHub для розгортання застосунків з ваших репозиторіїв. Для кращої безпеки та ізоляції рекомендується створювати Застосунки GitHub для конкретних команд.
                            </x-callout>
                        @endif
                    @endif
                    <div class="flex flex-col sm:flex-row gap-2">
                        ...
                        @if (!isCloud() || isDev())
                            <div class="flex flex-col sm:flex-row items-start sm:items-end gap-2">
                                <x-forms.select wire:model.live='webhook_endpoint' label="Кінцева точка вебхука"
                                    helper="Усі вебхуки Git будуть надсилатися на цю кінцеву точку. <br><br>Якщо ви бажаєте використовувати домен замість IP-адреси, встановіть FQDN вашого екземпляра AutoDeploy у меню Налаштувань.">
                                    @if ($ipv4)
