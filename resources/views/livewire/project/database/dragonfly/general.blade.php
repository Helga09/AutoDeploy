<div>
    <form wire:submit="submit" class="flex flex-col gap-2">
        ...
        <x-forms.input
            helper="Ви можете додати власні параметри запуску Docker, які будуть використані при запуску вашого контейнера.<br>Примітка: Не всі параметри підтримуються, оскільки вони можуть порушити автоматизацію AutoDeploy та спричинити поганий досвід для користувачів.<br><br>Перегляньте <a class='underline dark:text-white' href='https://AutoDeploy.io/docs/knowledge-base/docker/custom-commands'>документацію.</a>"
        ...
        @if ($database->started_at)
            <div class="flex gap-2">
                <x-forms.input label="Початковий пароль" id="dragonflyPassword" type="password" required readonly
        ...
        @else
            ...
            <div class="flex gap-2">
                <x-forms.input label="Пароль" id="dragonflyPassword" type="password" required canGate="update"
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="flex items-end gap-2">
                <x-forms.input placeholder="3000:5432" id="portsMappings" label="Мапування портів"
                    helper="Список портів, розділений комами, які ви хочете зіставити з хост-системою.<br><span class='inline-block font-bold dark:text-warning'>Приклад</span>3000:5432,3002:5433"
                    canGate="update" :canResource="$database" />
            </div>
            <x-forms.input label="URL Dragonfly (внутрішній)"
                helper="Якщо ви зміните користувача/пароль/порт, це може бути іншим. Це значення за замовчуванням."
                type="password" readonly wire:model="dbUrl" canGate="update" :canResource="$database" />

            @if ($dbUrlPublic)
                <x-forms.input label="URL Dragonfly (публічний)"
                    helper="Якщо ви зміните користувача/пароль/порт, це може бути іншим. Це значення за замовчуванням."
    ...
    <div class="w-64">
        <x-forms.checkbox helper="Надсилати журнали до налаштованої кінцевої точки зливу журналів у параметрах вашого сервера."
