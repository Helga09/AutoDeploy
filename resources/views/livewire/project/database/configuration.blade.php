<div>
    <x-slot:title>
        {{ data_get_str($database, 'name')->limit(10) }} > Конфігурація | AutoDeploy
    </x-slot>
    <h1>Конфігурація</h1>
    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        <div class="flex flex-col items-start gap-2 min-w-fit">
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.database.configuration', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'database_uuid' => $database->uuid]) }}">Загальне</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.database.environment-variables', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'database_uuid' => $database->uuid]) }}">Змінні
                оточення</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
                href="{{ route('project.database.servers', ['project_uuid' => $project->uuid, 'environment_uuid' => $environment->uuid, 'database_uuid' => $database->uuid]) }}">Сервери</a>
            <a class='menu-item' wire:current.exact="menu-item-active"
