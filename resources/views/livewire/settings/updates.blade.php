            ...
            <div class="flex flex-col gap-2">
                <div class="flex items-end gap-2">
                    <x-forms.input required id="update_check_frequency" label="Частота перевірки оновлень"
                        placeholder="0 * * * *"
                        helper="Частота (cron-вираз) для перевірки нових версій AutoDeploy та завантаження нових шаблонів сервісів з CDN.<br>Ви можете використовувати every_minute, hourly, daily, weekly, monthly, yearly.<br><br>За замовчуванням – щогодини." />
                ...
                <div class="text-right md:w-64">
                    @if (!is_null(config('constants.AutoDeploy.autoupdate', null)))
                        <div class="text-right">
                            <x-forms.checkbox instantSave
                                helper="Параметр AUTOUPDATE встановлено у файлі .env, вам потрібно змінити його там." disabled
                                checked="{{ config('constants.AutoDeploy.autoupdate') }}" label="Увімкнено" />
                        </div>
                    @else
                        <x-forms.checkbox instantSave id="is_auto_update_enabled" label="Увімкнено" />
                    @endif
                </div>
                @if (is_null(config('constants.AutoDeploy.autoupdate', null)) && $is_auto_update_enabled)
                    <x-forms.input required id="auto_update_frequency" label="Частота (cron-вираз)"
                        placeholder="0 0 * * *"
                        helper="Частота (cron-вираз) (автоматичне оновлення AutoDeploy).<br>Ви можете використовувати every_minute, hourly, daily, weekly, monthly, yearly.<br><br>За замовчуванням – щодня о 00:00" />
                @else
