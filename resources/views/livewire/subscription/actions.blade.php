<div>
    @if (subscriptionProvider() === 'stripe')
        <div class="pt-4">
            <h2>Ваш поточний план</h2>
            <div class="pb-4">Рівень: <strong class="dark:text-warning">
            ...
            @if (currentTeam()->subscription->stripe_cancel_at_period_end)
                <div class="pb-2">Підписка активна, але знаходиться в періоді скасування.</div>
            @else
                <div class="pb-2">Підписка активна. Останній рахунок {{ currentTeam()->subscription->stripe_invoice_paid ? 'сплачено' : 'не сплачено' }}.</div>
            @endif
            <div class="flex items-center gap-2">
                <div class="w-48">Кількість оплачених серверів:</div>
                <div class="text-xl font-bold dark:text-white">{{ $server_limits }}</div>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-48">Наразі активних серверів:</div>
                <div class="text-xl font-bold dark:text-white">{{ currentTeam()->servers->count() }}</div>
            </div>
            @if (currentTeam()->serverOverflow())
                <x-callout type="danger" title="ПОПЕРЕДЖЕННЯ" class="my-4">
                    Ви повинні видалити {{ currentTeam()->servers->count() - $server_limits }} серверів,
                    або оновити свою підписку. {{ currentTeam()->servers->count() - $server_limits }} серверів буде
                    деактивовано.
                </x-callout>
            @endif
            <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>Змінити кількість серверів
            </x-forms.button>
            <h2 class="pt-4">Керування вашою підпискою</h2>
            <div class="pb-4">Скасуйте, оновіть або понизьте свою підписку.</div>
            <div class="flex gap-2">
                <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>Перейти до <svg
        ...
        <div class="pt-4">
            Якщо у вас виникли проблеми, будь ласка, <a class="underline dark:text-white" href="{{ config('constants.urls.contact') }}"
